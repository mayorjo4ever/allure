...
<div class="row mt-0 pt-0">
        ...
        <table class="align-middle mb-0 table table-borderless table-striped table-hover dataTable">
            <thead>
            <tr>
                <th class="pl-4"># Type ID </th>
                <th>Name</th>                
                @can('modify-lenses-types')<th >Status</th> @endcan
                @can('edit-lenses-types') <th>Edit / Delete </th> @endcan
                <th>Last Updated </th>
            </tr>
            </thead>
            <tbody> @foreach($types as $type)
            <tr class="{{ ($type['status']==1) ?"active":"inactive" }}">
                <td class="text-muted pl-4"># {{ $type['id']}} </td>
                <td> {{ $type['name']}} </td>                
