        ...
        <div class="col-md-12"> 
            ...
            <x-admin.card header="{{$page_info['title']}}">
                <div class="table-responsive  mt-2 pt-2 filtered_customers ">
                    <table class="table table-bordered border-dark  dataTable" >
                        <thead>
                            <tr>                               
                                <th>Patient</th>
...
<!--                                <th>Services </th>
                        ...
                        <tbody>
                            ...
                            <tr id="row_{{ $appointment->id }}" class="border-dark">                                    
                                    ...
                                    <td> 
                                        ...
                                        <div id="checkin-{{$appointment->id}}"> <button class="btn btn-outline-primary p-1 btn-sm appointment-checkin-btn ladda-button" data-style="expand-right" data-id="{{ $appointment->id }}">Check-In Now </button> </div>
                                        @else
...
<!--                                    <td> @php $totalFee = 0; $amountPaid = 0;  @endphp
                                       @if(!empty($appointment->investigations->toArray()))
                                        @foreach($appointment->investigations as $investigation)
                                         ...
                                         <span class="badge table-secondary">
                                             {{$prescription->item->name  }} - {{ucwords($prescription->item_type)}}
                                         </span>
                                         @endforeach
