...
<div class="row mt-5">
    <div class="col-md-12">
        ...
        <strong><i> Your Next Appointment Date is : 
                ...
                <button disabled="" onclick="schedule_next_appointment('{{$appointment->doctor_id}}','{{$appointment->user_id}}','{{$appointment->id}}')"  data-toggle="modal" data-target="#next_appointment_modal"  class="btn btn-info btn-lg "> Set Up Next Appointment </button>
                @else 
                <?php
             $nx_appointment = appointment_details($appointment->next_appointment_id); 
                ...
                <span class="font-weight-500">{{ $nx_appointment->human_date }} </span>
                        <br/>   {{ $nx_appointment->appointment_date->format('D, jS F, Y') }}  &nbsp; 
                            {{ $nx_appointment->appointment_date->format('H:i A') }}
                          <br/>
                          <span class="countdown font-weight-500" style="font-size: 1.5rem"
    ...
    <div class="col-md-12">
       ...
       @php
        ...
        <div class="checkbox-wrapper-13 mb-3">
            <input disabled="" onclick="calc_other_bills()" id="c1-{{$bill->id}}" value="{{$bill->id}}" data-amount="{{$bill->amount}}" class="bills"  type="checkbox" name="bills[]"  @checked(in_array($bill->id, $my_bill_ids)) >
